class ScannerAbstract {
  constructor(editor, regex) {
    this.editor = editor;
    this.regex = regex;
    this.maxDepth = atom.config.get(
      "navigation-panel.general.maxHeadingDepth",
      {
  ...
  getHeaders() {
    let headers = [];
    let closeRow = this.editor.getLineCount();
    let lastItem = null;
    if (!this.editor) {
      return headers;
    }
    this.editor.scan(this.regex, {}, (object) => {
      ...
      if (lastItem) {
        lastItem.lastRow = item.startPoint.row - 1;
      }
      lastItem = item;
    });
    return headers;
...
module.exports = { ScannerAbstract };
