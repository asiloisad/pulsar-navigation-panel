...
class ScannerSofistik extends ScannerAbstract {
  ...
  constructor(editor) {
    super(editor, /^ *(#define [^\n=]+$|#enddef)|^!([+-\\#\\$])!(?:chapter|kapitel) (.*)|(^(?! *\$)[^!\n]*)!(\$+)!(.*)|^ *([+-])?prog +([^\n]*)|^ *!.! +(.*)|^\$ graphics +(\d+) +\| +picture +(\d+) +\| +layer +(\d+) +: *(.*)/gmi)
    this.defineCount = 0
  ...
  parse(object) {
    ...
    if (this.defineCount>0) {
      ...
      if (this.icons) {
        if (match[2]==='+') {
          iconClass = 'navigation-icon icon sofistik-navi-icon icon-unfold'
        } else if (match[2]==='-') {
          iconClass = 'navigation-icon icon sofistik-navi-icon icon-fold'
        } else {
          iconClass = 'navigation-icon icon sofistik-navi-icon icon-info'
        }
        display = <div class={iconClass} on={{click:(e)=>this.toggleChapter(e, object)}}/>
      }
    } else if (match[8]) {
      level = 5
      ...
      if (this.icons) {
        if (match[7]==='+') {
          iconClass = 'navigation-icon icon sofistik-navi-icon status-added icon-diff-added'
        } else if (match[7]==='-') {
          iconClass = 'navigation-icon icon sofistik-navi-icon status-removed icon-diff-removed'
        } else {
          iconClass = 'navigation-icon icon sofistik-navi-icon status-modified icon-diff-modified'
        }
        display = <div class={iconClass} on={{click:(e)=>this.toggleProg(e, object)}}/>
      } else {
        iconClass = null
      }
    } else if (match[9]) {
      ...
      // display = [<span class='badge badge-flexible'>{text1}</span>, <span>{text2}</span>]
    } else {
      return
    }
  ...
  toggleChapter(e, object) {
    if (object.match[2]==='+') {
      ...
      e.srcElement.className = 'navigation-icon icon sofistik-navi-icon icon-fold'
    } else if (object.match[2]==='-') {
      object.replace(object.matchText.replace(/!\-!/i, '!+!'))
      e.srcElement.className = 'navigation-icon icon sofistik-navi-icon icon-unfold'
    }
  }
  ...
  toggleProg(e, object) {
    if (object.match[7]==='+') {
      ...
      e.srcElement.className = 'navigation-icon icon sofistik-navi-icon status-removed icon-diff-removed'
    } else if (object.match[7]==='-') {
      object.replace(object.matchText.replace(/\-(prog)/i, '+$1'))
      e.srcElement.className = 'navigation-icon icon sofistik-navi-icon status-added icon-diff-added'
    }
  }
}
