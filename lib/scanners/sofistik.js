  ...
  parse(object) {
    ...
    if (this.defineCount > 0) {
      ...
      if (this.icons) {
        if (match[7] === "+") {
          iconClass =
            "navigation-icon icon sofistik-navi-icon status-added icon-diff-added";
        } else if (match[7] === "-") {
          iconClass =
            "navigation-icon icon sofistik-navi-icon status-removed icon-diff-removed";
        } else {
          iconClass =
            "navigation-icon icon sofistik-navi-icon status-modified icon-diff-modified";
        }
        display = (
          <div
            class={iconClass}
  ...
  toggleChapter(e, object) {
    if (object.match[2] === "+") {
      ...
      e.srcElement.className =
        "navigation-icon icon sofistik-navi-icon icon-fold";
    } else if (object.match[2] === "-") {
      object.replace(object.matchText.replace(/!\-!/i, "!+!"));
      e.srcElement.className =
        "navigation-icon icon sofistik-navi-icon icon-unfold";
    }
  }
