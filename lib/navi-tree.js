...
class NavigationTree {
  ...
  render() {
    ...
    searchBar = <div class='navigation-search' style={
      STATES.searchBar ? 'display:block' : 'display:none'
    }>{
    ...
    if (!items) {
      ...
      naviList = <div class="navigation-list" style={`white-space:${wspace};`}>{
        items.map((item) => {
  ...
  scrollToElement(element) {
    ...
    if (this.instant) {
      this.instant = false
      let container = document.querySelector('.navigation-list')
      if (!container) { return }
    ...
    const animate = () => {
      ...
      let step = Math.trunc(this.pendingScroll / 8)
  ...
  scrollToLine(e) {
    if (this.item.viewer) {
      ...
      if (STATES.animationThreshold && this.item.editor.getElement().scrollAnimation) {
        let diff = Math.abs(
          this.item.editor.getCursorScreenPosition().row
          -
          this.item.editor.screenPositionForBufferPosition(
            [this.item.startPoint.row, 0]).row
        )
        this.item.editor.setCursorBufferPosition(
          [this.item.startPoint.row, 0], { autoscroll: false }
        )
        if (diff <= STATES.animationThreshold) {
          this.item.editor.getElement().scrollAnimation(
            { lastCursor: true, center: STATES.centerScroll }
          )
        } else {
