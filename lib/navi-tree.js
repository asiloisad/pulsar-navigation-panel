...
class NavigationTree {
  constructor() {
    ...
    STATES.categoryBar = atom.config.get(
      "navigation-panel.general.categoryBar"
    );
    ...
    this.disposables = new CompositeDisposable(
      atom.config.observe(
        "navigation-panel.general.centerScroll",
        {},
        (value) => {
        ...
        <div class="navigation-list" style={{ width: wspace }}>
          {items.map((item) => {
            return (
              <TreeView
                {...item}
                key={item.startPoint ? item.startPoint.row : item.text}
              />
            );
          })}
        </div>
  ...
  update(headers, props) {
    ...
    if (props) {
      if (props.hasOwnProperty("instant")) {
        this.instant = props.instant;
      }
      if (props.hasOwnProperty("scrollDirection")) {
  ...
  _filter(query, items, headers) {
    for (let item of headers) {
      let score = atom.ui.fuzzyMatcher.score(
        Diacritics.clean(item.text),
        query
      );
      if (score > 0) {
        let matches =
        ...
        items.push({
          ...
          filterRow: item.startPoint ? item.startPoint.row : null,
        });
      }
...
class TreeView {
  constructor(item) {
    ...
    if (STATES.visibility === 2 || STATES.collapseWork === 2) {
      if (this.item.viewer) {
        ...
        this.showChildren =
          this.item.visibility > 0 || this.checkChildrenVisibility(this.item);
  ...
  render() {
    ...
    let naviClass = this.item.classList.length
      ? " " + this.item.classList.join(" ")
      : "";

    return (
      <div class={"navigation-tree" + stackClass}>
        <div
          class={"navigation-block" + naviClass + currentClass + visibleClass}
        >
          <div
  ...
  checkChildrenVisibility(item) {
    return (
      item.visibility ||
      !!item.children.filter((child) => this.checkChildrenVisibility(child))
        .length
    );
  }
...
function scrollToCursorWithMode(editor, mode) {
  if (mode === "center") {
    ...
    const scrollTop = editorView.component.pixelPositionAfterBlocksForRow(
      cursorRow
    );
