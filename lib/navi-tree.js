...
class NavigationTree {
  ...
  render() {
    ...
    if (!items) {
      ...
      naviList = (
        <div class="navigation-list" style={{ width: wspace }}>
          {items.map((item) => {
            return <TreeView {...item} key={item.startPoint.row} />;
          })}
        </div>
      );
    } else {
    ...
    return (
      <atom-panel class="navigation-panel">
        {searchBar}
        <div class="navigation-scroller">{naviList}</div>
        {categoryBar}
      </atom-panel>
    );
  }
...
class TreeView {
  constructor(item) {
    ...
    if (STATES.visibility === 2 || STATES.collapseWork === 2) {
      if (this.item.viewer) {
        // PDF: expand based on visibility
        this.showChildren = this.item.visibility > 0 || this.checkChildrenVisibility(this.item);
      } else if (this.item.stackCount > 0) {
  ...
  render() {
    ...
    return (
      <div class={"navigation-tree" + stackClass}>
        <div class={"navigation-block" + naviClass + currentClass + visibleClass}>
          ...
          {this.item.filterResult && this.item.viewer && this.item.resolvedDest ? (
            <span class="badge badge-flexible">{this.item.resolvedDest.pageIndex + 1}</span>
  ...
  scrollToLine(e) {
    if (this.item.viewer) {
      ...
      // scrollToDestination fires observeCurrent synchronously before returning.
      // The iframe then fires it again async with the stale position before navigating.
      // Flag is set here (after the synchronous callback already ran) so only the
      // stale async callback gets skipped.
      this.item.viewer._skipNextCurrent = true;
      atom.views.getView(this.item.viewer).focus();
      return;
    } else if (this.item.filePath) {
      // Handle image-editor file navigation
      if (e.altKey) {
        atom.clipboard.write(this.item.text);
        atom.notifications.addSuccess("Filename has been copied!");
      } else {
        // Load the image file
        this.item.editor.load(this.item.filePath);
      }
      ...
      SCROLL_SKIP += 1;
      this.item.editor.setCursorBufferPosition([this.item.startPoint.row, 0], {
        autoscroll: false,
      });
      scrollToCursorWithMode(this.item.editor, STATES.centerScroll);
    }
    atom.views.getView(this.item.editor).focus();
  }
...
function scrollToCursorWithMode(editor, mode) {
  if (mode === "center") {
    editor.scrollToCursorPosition({ center: true });
  } else if (mode === "top") {
    const editorView = atom.views.getView(editor);
    const cursorRow = editor.getCursorScreenPosition().row;
    const scrollTop = editorView.component.pixelPositionAfterBlocksForRow(cursorRow);
    const margin = editorView.component.getVerticalAutoscrollMargin();
    editorView.setScrollTop(scrollTop - margin * 5);
  } else if (mode === "top-half") {
    const editorView = atom.views.getView(editor);
    const cursorRow = editor.getCursorScreenPosition().row;
    const cursorPixel = editorView.component.pixelPositionAfterBlocksForRow(cursorRow);
    const scrollTop = editorView.getScrollTop();
    const halfHeight = editorView.getHeight() / 2;
    if (cursorPixel - scrollTop >= halfHeight) {
      editorView.setScrollTop(cursorPixel - halfHeight);
    } else if (cursorPixel < scrollTop) {
      const margin = editorView.component.getVerticalAutoscrollMargin();
      editorView.setScrollTop(cursorPixel - margin);
    }
  } else {
    // "nearest" - minimal scroll, just ensure cursor is visible
    editor.scrollToCursorPosition({ center: false });
  }
}

module.exports = { NavigationTree, scrollToCursorWithMode };
