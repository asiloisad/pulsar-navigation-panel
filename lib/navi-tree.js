  ...
  _filter(query, items, headers) {
    for (let item of headers) {
      ...
      if (score > 0) {
        ...
        items.push({ ...item, score: score, children: [], filterResult: display })
      }
  ...
  render() {
    ...
    return <div class={"navigation-tree" + stackClass}>
      <div class={"navigation-block" + naviClass + currentClass + visibleClass}>
        <div class={"navigation-icon navigation-state-icon icon" + iconClass} on={{ click: this.toggleNested }} />
        {this.item.display ? this.item.display : ""}
        <div class="navigation-text" on={{ click: this.scrollToLine }}>{this.item.filterResult ? this.item.filterResult : this.item.text}</div>
      </div>
      {naviList}
    </div>
  }
