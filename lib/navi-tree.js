...
class NavigationTree {
    ...
    for (let item of headers) {
      ...
      if (score > 0) {
        ...
        let display = this.highlightMatchesInElement(
          item.text,
          matches,
          item.startPoint ? item.startPoint.row : null
        );
        items.push({
          ...item,
          score: score,
  ...
  highlightMatchesInElement(text, matches, row) {
    let el =
      row !== null ? [<span class="badge badge-flexible">{row + 1}</span>] : [];
    let matchedChars = [];
    let lastIndex = 0;
  ...
  render() {
    ...
    return (
      <div class={"navigation-tree" + stackClass}>
        ...
        >
          <div
            class={"navigation-icon navigation-state-icon icon" + iconClass}
...
function scrollToCursorWithMode(editor, mode) {
  if (mode === "center") {
    ...
    editorView.setScrollTop(scrollTop - margin*2);
  } else {
