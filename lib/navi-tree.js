...
const { compute } = require('compute-scroll-into-view')
...
class NavigationTree {
  ...
  render() {
    ...
    searchBar = <div class='navigation-search' style={STATES.searchBar ? 'display:block' : 'display:none'}>{
  ...
  scrollToElement(element) {
    ...
    return compute(element, {
      ...
      if (el.classList.contains('navigation-list')) {
        el.scrollTo({ top: top, behavior: behaviour })
      }
    })
  }
    ...
    if (this.item.viewer) {
      ...
      if (STATES.animationThreshold && this.item.editor.getElement().scrollAnimation) {
        let diff = Math.abs(
          ...
          -
          this.item.editor.screenPositionForBufferPosition([this.item.startPoint.row, 0]).row
        )
