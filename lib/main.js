...
module.exports = {
  ...
  ensureMarkerLayer(editor, index) {
    ...
    if (!buffer.navigationMarkerLayers[index]) {
      buffer.navigationMarkerLayers[index] = buffer.addMarkerLayer({ role: `navigation-marker-${index}` })
  ...
  decorateMarkerLayer(editor, layer, index) {
    for (let decoType of this.markerType.split('&')) {
      editor.decorateMarkerLayer(layer, {
        type: decoType, class: `navigation-marker-${index} navigation-marker ${decoType}-decoration`
      })
    }
  },
