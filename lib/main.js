...
module.exports = {
      ...
      if (scopeName in SCANNERS) {
        ...
        this.editorDispose = this.editor.onDidStopChanging(() => {
          ...
          for (let range of this.changedRanges) {
            if (
              (range.start.row <= lastVisibleRow && range.end.row >= firstVisibleRow)
            ) {
              update = true
              break
            }
  ...
  isImageEditor(item) {
    // Check if item is an ImageEditor by looking for unique properties
    // ImageEditor has: file, getEncodedURI(), load(), and a view getter
    const result = item && 
    ...
    if (result) {
      console.log('navigation-panel: Detected ImageEditor', item)
    }
  ...
  imageEditorSubscribe(imageEditor) {
    console.log('navigation-panel: imageEditorSubscribe called', imageEditor)
    this.editorUnsubscribe()
    this.editor = imageEditor
    // Get the view from the editor
    this.editorView = imageEditor.view
    
    console.log('navigation-panel: Got editorView', this.editorView)
    
    if (!this.editorView) {
      console.warn('navigation-panel: Could not get image editor view')
      return
    }
    ...
    console.log('navigation-panel: Created scanner, calling getHeaders')
    const headers = this.scanner.getHeaders()
    console.log('navigation-panel: Got headers', headers)
    ...
    this.editorDispose.add(
      imageEditor.onDidChange(() => {
        console.log('navigation-panel: onDidChange triggered')
        this.update({ instant: true })
      })
    )
    ...
    this.editorDispose.add(
      imageEditor.onDidReplaceFile(() => {
        console.log('navigation-panel: onDidReplaceFile triggered')
        this.update({ instant: true })
      })
    )
    
    console.log('navigation-panel: Calling initial update')
    this.update({ instant: true })
  },
    ...
    try {
      if (!this.scanner) {
        ...
        try {
          ...
          if (this.editorView && typeof this.editorView.getScrollTop === 'function') {
          ...
          console.error('navigation-panel: Error in update', err)
        }
      }
  ...
  markVisibleItems(scrollTop) {
    if (this.traceVisible && this.headers) {
      ...
      let rowTop = this.editorView.screenPositionForPixelPosition(
        { top: scrollTop, left: 0 }).row
      let rowBot = this.editorView.screenPositionForPixelPosition(
        { top: scrollTop + editorHeight, left: 0 }).row
    ...
    for (var item of headers) {
      ...
      if (layer) {
        layer.markRange(
          ...
          { exclusive: true, invalidate: 'inside' }
        )
    ...
    if (!buffer.navigationMarkerLayers[index]) {
      ...
      for (let ed of atom.workspace.getTextEditors()) {
        if (ed.getBuffer() === buffer) {
          this.decorateMarkerLayer(ed,
            buffer.navigationMarkerLayers[index], index
          )
        }
      }
    }
  ...
  _collectVisibleHeaders(items, headers) {
    for (const header of headers) {
      ...
      let visible = true
      if (header.classList) {
        if (header.classList.includes('info') && !atom.config.get('navigation-panel.categories.info')) visible = false
        if (header.classList.includes('success') && !atom.config.get('navigation-panel.categories.success')) visible = false
        if (header.classList.includes('warning') && !atom.config.get('navigation-panel.categories.warning')) visible = false
        if (header.classList.includes('error') && !atom.config.get('navigation-panel.categories.error')) visible = false
        if (header.classList.length === 0 && !atom.config.get('navigation-panel.categories.standard')) visible = false
      }
  ...
  navigateToHeader(item) {
    ...
    if (item.viewer) {
      ...
      item.editor.setCursorBufferPosition(
        [item.startPoint.row, 0], { autoscroll: false }
      )
      item.editor.scrollToCursorPosition(
        { center: atom.config.get('navigation-panel.general.centerScroll') }
      )
