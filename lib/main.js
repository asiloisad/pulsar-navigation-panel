...
const { NavigationList } = require("./list");
...
module.exports = {
      ...
      if (scopeName in SCANNERS) {
        ...
        this.editorDispose = this.editor.onDidStopChanging(() => {
          ...
          for (let range of this.changedRanges) {
            if (range.start.row <= lastVisibleRow && range.end.row >= firstVisibleRow) {
              update = true;
              break;
            }
          }
  ...
  viewerSubscribe(viewer) {
    ...
    this.currentDispose = viewer.observeCurrent((destHashes) => {
      ...
      // Consume the skip flag regardless of traceVisible so it never leaks.
      // Set in scrollToLine after scrollToDestination() returns (i.e. after the
      // synchronous callback has already run), so only the stale iframe callback
      // that fires before the PDF has actually navigated gets dropped.
      if (viewer._skipNextCurrent) {
        viewer._skipNextCurrent = false;
        return;
      }
      if (!this.traceVisible) {
        return;
    ...
    if (!buffer.navigationMarkerLayers[index]) {
      ...
      for (let ed of atom.workspace.getTextEditors()) {
        if (ed.getBuffer() === buffer) {
          this.decorateMarkerLayer(ed, buffer.navigationMarkerLayers[index], index);
        }
      }
    }
  ...
  isHeaderVisible(header) {
    ...
    if (header.classList.includes("info") && !atom.config.get("navigation-panel.categories.info"))
      return false;
    if (
    ...
    if (
      ...
      return false;
    if (header.classList.includes("error") && !atom.config.get("navigation-panel.categories.error"))
      return false;
    if (header.classList.length === 0 && !atom.config.get("navigation-panel.categories.standard"))
      return false;
    return true;
  },
  ...
  navigateToHeader(item, options = {}) {
    ...
    if (item.viewer) {
      ...
      scrollToCursorWithMode(
        item.editor,
        atom.config.get("navigation-panel.general.editorScrollPosition"),
      );
      if (focus) {
