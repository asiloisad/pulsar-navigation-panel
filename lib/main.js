...
module.exports = {
  ...
  /**
   * Provides the navigation-panel service API.
   * @returns {Object} Service with header access and observation methods
   */
  serviceProvider() {
    return {
      getFlattenHeaders: () => {
  ...
  /**
   * Consumes the scroll-map service to register navigation layer.
   * @param {Object} scrollMapService - The scroll-map service
   * @returns {Disposable} Disposable to unregister the layer
   */
  consumeScrollMap(scrollMapService) {
    const mainModule = this;

    class NaviLayer extends scrollMapService.ScrollMapLayer {
      constructor(editor) {
        super({ editor: editor, name: "navi", timer: 50 });
        this.threshold = 0;
        this.disposables.add(
          mainModule.observeHeaders(() => this.update()),
          atom.config.observe("navigation-panel.scrollMapThreshold", (value) => {
      ...
      recalculate() {
        ...
        if (
          !mainModule.editor ||
          !mainModule.editor.buffer ||
          mainModule.editor.buffer.id !== this.editor.buffer.id
        ) {
          return;
        }
        if (!mainModule.headers) {
          return;
        }
        let headers = mainModule.getFlattenHeaders();
    ...
    const updateLayer = () => {
      if (atom.config.get("navigation-panel.scrollMapState")) {
        scrollMapService.registerLayer("navi", NaviLayer);
      } else {
        scrollMapService.unregisterLayer("navi");
      }
    };

    updateLayer();
    const configDisposable = atom.config.observe(
      "navigation-panel.scrollMapState",
      updateLayer
    );
